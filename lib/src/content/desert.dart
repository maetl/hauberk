...
class Desert extends StageBuilder {
  ...
  ///
  /// Fewer iterations leads to more spiky terrain, with a greater likelihood of
  ...
  void generate(Stage stage) {
    ...
    for (var i = 0; i < generatorIterations; i++) {
      for (var y = 1; y < stage.height - 1; y++) {
        for (var x = 1; x < stage.width - 1; x++) {
          ...
          if (getTile(cell) == Tiles.canyon &&
              adjacentWalls < wallToFloorUpperThreshold) {
            setTile(cell, Tiles.sand);
          } else if (getTile(cell) == Tiles.sand &&
              adjacentWalls > floorToWallLowerThreshold) {
    ...
    for (var y = 1; y < stage.height - 1; y++) {
      for (var x = 1; x < stage.width - 1; x++) {
        ...
        if ((getTile(cell.offset(0, -1)) == Tiles.sand &&
                getTile(cell.offset(0, 1)) == Tiles.sand) ||
            (getTile(cell.offset(1, 0)) == Tiles.sand &&
                getTile(cell.offset(-1, 0)) == Tiles.sand)) {
          setTile(cell, Tiles.sand);
        }
      }
  ...
  int countAdjacentWalls(Vec pos) {
    ...
    for (var dir in Direction.ALL) {
      if (getTile(pos.offset(dir.x, dir.y)) == Tiles.canyon) walls++;
    }
